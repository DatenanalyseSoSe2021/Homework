...
my.cv.glm = function(formula, data, k, family = gaussian){
  ...
  for(i in 1:k){
    ...
    #Store RSS of the fit
    cv.err[i] = sum(fit.glm$residual^2)
  }
  return(cv.err)
}
...
(my.data.RSSs = my.cv.glm(y~x,data=my.data,k = 10))
mean(my.data.RSSs)
...
library(titanic)
data("titanic_train")
(titanic.RSSs = my.cv.glm(Survived ~ Pclass+Sex+Age, data = titanic_train,k=5, family= binomial))
mean(titanic.RSSs)
